...
def set_hparams(config='', exp_name='', hparams_str='', print_hparams=True, global_hparams=True):
    if config == '' and exp_name == '':
        ...
        parser.add_argument('--naive_fine_tune', action='store_true', help='fine tune only speaker_id_proj')
        parser.add_argument('--single_speaker_ada_fine_tune', action='store_true', help='adaspeech style fine tuning on a dataset with a single speaker')
        parser.add_argument('--fine_tune_weight_sel', type=str, default='', help='choose which adaspeech style fine tuned weights to load in. String of letters with no spaces. e=encoder, d=decoder, l = dur predictor, p=pitch predictor, s=speaker embedding. If none provided, will load all')
        args, unknown = parser.parse_known_args()
        print("| Unknow hparams: ", unknown)
    else:
        args = Args(config=config, exp_name=exp_name, hparams=hparams_str,
                    infer=False, validate=False, reset=False, debug=False, remove=False,naive_fine_tune=False,single_speaker_ada_fine_tune=False,fine_tune_weight_sel='')
    global hparams
    ...
    hparams_['naive_fine_tune']=args.naive_fine_tune
    hparams_['single_speaker_ada_fine_tune']=args.single_speaker_ada_fine_tune
    hparams_['fine_tune_weight_sel']=args.fine_tune_weight_sel
