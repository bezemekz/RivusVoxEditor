...
class Trainer:
    ...
    def run_training_batch(self, batch_idx, batch):
        ...
        for opt_idx, optimizer in enumerate(self.optimizers):
            ...
            if len(self.optimizers) > 1:
                ...
                if hparams['naive_fine_tune']:
                    fine_tune_params=[param for param in task_ref.model.named_parameters() if param[0]=='fs.spk_id_proj.weight']
                    if len(fine_tune_params)!=1:
                        sys.exit('Fine tuning paramater not found in model, make sure use_spk_id==True') 
                    num_fine_tuning_params=0
                    for param in task_ref.model.named_parameters():
                        if param in fine_tune_params:
                            num_fine_tuning_params+=1
                            param.requires_grad = True
                        else:
                            param.requires_grad = False
                    if num_fine_tuning_params!=1:
                        sys.exit('Fine tuning paramater not found in model, make sure use_spk_id==True') 
                    req_grad_params=[param for param in task_ref.model.parameters() if param.requires_grad]
                    if len(req_grad_params)!=1:
                        sys.exit('Failed to set all fine-tuning params to requires_grad=False')
                    num_optimizer_params=0
                    for group in optimizer.param_groups:
                        for param in group['params']:
                            num_optimizer_params+1
                    if len(num_optimizer_params)!=1:
                        for group in optimizer.param_groups:
                            for param in group['params']:
                                print(param.shape)
                        sys.exit('Optimizer seeing too many paramaters!')
                elif hparams['single_speaker_ada_fine_tune']:
                    fine_tune_params=[param for param in task_ref.model.named_parameters() if param[0] in task_ref.fine_tuning_param_names]
                    if len(fine_tune_params)!=len(task_ref.fine_tuning_param_names):
                        sys.exit('Fine tuning paramater not found in model') 
                    num_fine_tuning_params=0
                    for param in task_ref.model.named_parameters():
                        if param in fine_tune_params:
                            num_fine_tuning_params+=1
                            ...
                            param.requires_grad = False
                    if num_fine_tuning_params!=len(task_ref.fine_tuning_param_names):
                        sys.exit('Fine tuning paramater not found in model') 
                    req_grad_params=[param for param in task_ref.model.parameters() if param.requires_grad]
                    if len(req_grad_params)!=len(task_ref.fine_tuning_param_names):
                        sys.exit('Failed to set all fine-tuning params to requires_grad=False')
                    num_optimizer_params=0
                    for group in optimizer.param_groups:
                        for param in group['params']:
                            num_optimizer_params+1
                    if len(num_optimizer_params)!=len(task_ref.fine_tuning_param_names):
                        for group in optimizer.param_groups:
                            for param in group['params']:
                                print(param.shape)
                        sys.exit('Optimizer seeing too many paramaters!')
    ...
    def restore_weights(self, checkpoint):
        ...
        if (hparams['naive_fine_tune']) or hparams['single_speaker_ada_fine_tune']:
            ckpt_paths = get_all_ckpts(self.work_dir,None) 
            if len(ckpt_paths) > 0:
                orig_ckpt_path = ckpt_paths[-1] #assumes you want to fine tune the oldest checkpoint in the working directory
                orig_checkpoint = torch.load(orig_ckpt_path, map_location='cpu')
                print(f'Fine tuning on {orig_ckpt_path}')
                for k, v in orig_checkpoint['state_dict'].items():
                    getattr(task_ref, k).load_state_dict(v,strict=False)
            else:
                sys.quit('No checkpoint found to fine tune on')
            if len(ckpt_paths) > 1:
                dict_to_load={k:v for k,v in checkpoint['state_dict'].items() if k in task_ref.fine_tuning_param_names}
                task_ref.model.load_state_dict(dict_to_load,strict=False)
                print(dict_to_load.keys())
            else:
                print('Warning: No previous fine-tuned weights found. If this is your first run of fine tuning, this is okay')
        else:
    ...
    def restore_opt_state(self, checkpoint):
        ...
        for optimizer, opt_state in zip(self.optimizers, optimizer_states):
            if optimizer is None:
                return
            try:
                if not (hparams['naive_fine_tune'] or hparams['single_speaker_ada_fine_tune']):
    ...
    def dump_checkpoint(self):
        ...
        if hparams['naive_fine_tune']:
            checkpoint['state_dict'] = {
                k: v for k, v in task_ref.model.named_parameters() if  k=='fs.spk_id_proj.weight'}
        elif hparams['single_speaker_ada_fine_tune']:
            checkpoint['state_dict'] = {
                k: v for k, v in task_ref.model.named_parameters() if  k in task_ref.fine_tuning_param_names}
        else:
            checkpoint['state_dict'] = {
    ...
    def save_codes(self):
        if len(hparams['save_codes']) > 0:
            ...
            for c in hparams['save_codes']:
                if os.path.exists(c):
                    subprocess.check_call(f'robocopy "{c}" "{code_dir}" *.py *.yaml /S /XD __pycache__ /XF *.* /XD * /E',shell=True)
