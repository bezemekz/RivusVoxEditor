...
def get_align_from_mfa_output(tg_fn, ph, ph_token, mel,wav,use_MFA,processor,align_model,device, text2mel_params={'hop_size':256,'audio_sample_rate':22050, 'mfa_min_sil_duration':0.1}):
    
    if (tg_fn is not None and os.path.exists(tg_fn)) or (not use_MFA):
        mel2ph, dur,sil_frames = get_mel2ph(tg_fn, ph, mel, text2mel_params['hop_size'], text2mel_params['audio_sample_rate'],wav,use_MFA,
                                    processor,align_model,device,text2mel_params['mfa_min_sil_duration'])
    else:
