...
class SpeechBaseTask(BaseTask):
    ...
    def build_optimizer(self, model):
        if hparams['naive_fine_tune']:
            fine_tune_params=[param[1] for param in model.named_parameters() if param[0]=='fs.spk_id_proj.weight']
            ...
            fine_tune_params=[param[1] for param in model.named_parameters() if param[0] in self.fine_tuning_param_names]
            if len(self.fine_tuning_param_names)!=len(fine_tune_params):
    ...
    def test_step(self, sample, batch_idx):
        ...
        self.saving_result_pool.add_job(self.save_result, args=[
            wav_pred, mel_pred, base_fn % 'P', gen_dir,hparams, str_phs])
        if hparams['save_gt']:
            wav_gt = self.vocoder.spec2wav(mel_gt)
            self.saving_result_pool.add_job(self.save_result, args=[
                wav_gt, mel_gt, base_fn % 'G', gen_dir,hparams, str_phs])
    ...
    @staticmethod
    def save_result(wav_out, mel, base_fn, gen_dir,hparams, str_phs=None, mel2ph=None, alignment=None):
