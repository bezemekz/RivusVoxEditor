...
class GaussianDiffusion(nn.Module):
    def __init__(self, phone_encoder, out_dims, denoise_fn,
                 timesteps=1000, time_scale=1, loss_type='l1', betas=None, spec_min=None, spec_max=None,hparams=hparams):
        super().__init__()
    ...
    def forward(self, txt_tokens, time_mel_masks, mel2ph, spk_embed,
                ref_mels, f0, uv, spk_id=None,
                infer=False, use_pred_mel2ph=False, use_pred_pitch=False):
        b, *_, device = *txt_tokens.shape, txt_tokens.device
        ret = {}
        ret = self.fs(txt_tokens, time_mel_masks, mel2ph, spk_embed, f0, uv, spk_id=spk_id,
        ...
        if not infer:
            ...
            #denoise_diff_start_time= time.time()
            t = self.num_timesteps  # reverse time steps
            shape = (cond.shape[0], 1, self.mel_bins, cond.shape[2])
            x = torch.randn(shape, device=device)  # noise
            #single_step_times=np.zeros(t)
            #all_specs=[]
            ...
            for i in reversed(range(0, t)):
                ...
                #single_step_times[i]=time.time()-step_time_start
                #all_specs.append(x[:, 0].transpose(1, 2))
            x = x[:, 0].transpose(1, 2)
            ret['mel_out'] = self.denorm_spec(x)  # denorm
            #print(f'Denoising diffusion total time: {time.time()-denoise_diff_start_time}')
            #print(f'Times for a single diffusion step: {single_step_times}')
            #Z: this is where the sde sampling happens. The posterior distribution is determined by the beta, which interestingly are set by default to schedule_type = vpsde (variance preserving sde), not cosine, so are determined by vpsde_beta_t in diffusion_utils.py
            #where does this hparam come from? did they really train with cosine schedule? If so, why do inference with a different noise schedule?
        return ret#,all_specs
